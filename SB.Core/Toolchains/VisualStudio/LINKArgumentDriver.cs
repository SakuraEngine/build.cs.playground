...
namespace SB.Core
{
    ...
    public class LINKArgumentDriver : IArgumentDriver
    {
        ...
        [TargetSetter] public string[]? LinkDirs(string[] dirs) => dirs.All(x => VS.CheckPath(x, false) ? true : throw new ArgumentException($"Invalid link dir {x}!")) ? dirs.Select(dir => $"/LIBPATH:{dir}").ToArray() : null;

        [TargetSetter] public string[]? WholeArchive(string[] libs) => libs.Select(lib => $"/WHOLEARCHIVE:{lib}").ToArray();
        ...
        public string[]? Inputs(string[] inputs) => inputs;
