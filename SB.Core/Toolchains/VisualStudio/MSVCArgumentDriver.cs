...
namespace SB.Core
{
    public class MSVCArgumentDriver : IArgumentDriver
    {
        ...
        [Argument] public string PDBMode(bool use) => use ? "/Zi" : "";

        [Argument] public string PDB(string path) => Path.IsPathFullyQualified(path) ? $"/Fd{path}" : throw new ArgumentException($"PDB value {path} is not an existed absolute path!");
        ...
        [Argument] public string[]? IncludeDirs(string[] dirs) => dirs.All(x => Path.IsPathFullyQualified(x) ? true : throw new ArgumentException($"Invalid include dir {x}!")) ? dirs.Select(dir => $"/I{dir}").ToArray() : null;

        [Argument] public string RTTI(bool v) => v ? "/GR" : "/GR-";

        [Argument] public string Object(string path) => Path.IsPathFullyQualified(path) ? $"/Fo{path}" : throw new ArgumentException($"Object value {path} is not an existed absolute path!");
